...
уже полученные ответы и не запрашивает их заново без нужды, а заодно убирает
запросы, результат которых нигде не показывается: селектор спидрана качает
30 таблиц рекордов, а рисует одну из них, и вся библиотека воркшопа (включая
превью-картинки всех подписанных карт) грузится на входе в главное меню, даже
если экран воркшопа ни разу не открывать. На видимое поведение игры это не
влияет — свой новый рекорд, как и раньше, появляется сразу.
